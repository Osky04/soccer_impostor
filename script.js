document.addEventListener('DOMContentLoaded', () => {
    ...
    function startGame() {
        ...
        // Actualizar instrucciones (sin el mensaje del jugador a nombrar)
    ...
    function createCards(gameData) {
        ...
        gameData.forEach(player => {
            ...
            const viewedMark = document.createElement('div');
            viewedMark.className = 'card-viewed-mark';
            viewedMark.innerHTML = '&#10003;'; // Checkmark symbol
            cardFront.appendChild(viewedMark);
            ...
            card.addEventListener('click', () => {
                // Prevent flipping if already viewed or currently flipping
                if (card.classList.contains('viewed') || card.dataset.flipping === 'true') {
                    return;
                }

                card.classList.add('flipped'); // Flip to show role
                card.dataset.flipping = 'true'; // Mark as currently flipping

                setTimeout(() => {
                    card.classList.remove('flipped'); // Flip back to front
                    card.classList.add('viewed');     // Mark as viewed permanently
                    delete card.dataset.flipping;     // Clear flipping state
                }, 1800); // Duration matches CSS transition for consistency
            });
